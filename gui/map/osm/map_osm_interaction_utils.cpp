...
bool map_osm_handle_press(const QPoint &pos, Qt::MouseButton button,
  ...
  if (button == Qt::LeftButton) {
    ...
    if (running_ui && !rects.osm_launch_btn_rect.isEmpty() &&
        rects.osm_launch_btn_rect.contains(pos)) {
      if (actions.launch_signal) actions.launch_signal();
      if (actions.update_all) actions.update_all();
      return true;
    }
...
bool map_osm_handle_wheel(const QPoint &pos, int delta,
  ...
  const int max_zoom = std::max(2, zoom_max);
  int new_zoom = std::max(2, std::min(max_zoom, old_zoom + zoom_steps));
