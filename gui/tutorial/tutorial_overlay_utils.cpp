...
void tutorial_draw_overlay(QPainter &p, const TutorialOverlayInput &in,
  ...
  QPixmap snapshot;
  if ((step == 5 || step == 6 || step == 7 || step == 8) && !in.signal_clean_snapshot.isNull() &&
  ...
  } else if (step == 2 && !in.osm_lower_clean_snapshot.isNull() &&
      !in.osm_lower_clean_rect.isEmpty()) {
    snapshot = in.osm_lower_clean_snapshot;
    focus_grab = in.osm_lower_clean_rect;
  } else if (!s_frozen_background.isNull() && !focus_grab.isEmpty()) {
  ...
  if (show_radial_callouts) {
    if (step == 1) {
      // Map Upper Half: five map controls explained.
      callouts.push_back({"search_box", tr_key("tutorial.callout.step1.search_box"), 268.0, 352.0, QSize(220, 92)});
      callouts.push_back({"nfz_btn", tr_key("tutorial.callout.step1.nfz_btn"), 232.0, 330.0, QSize(220, 92)});
      callouts.push_back({"dark_mode_btn", tr_key("tutorial.callout.step1.dark_mode_btn"), 304.0, 330.0, QSize(240, 92)});
      callouts.push_back({"guide_btn", tr_key("tutorial.callout.step1.guide_btn"), 18.0, 330.0, QSize(220, 92)});
      callouts.push_back({"lang_btn", tr_key("tutorial.callout.step1.lang_btn"), 92.0, 330.0, QSize(220, 92)});
    } else if (step == 2) {
      // Map Lower Half custom explanations.
      callouts.push_back({"zoom", tr_key("tutorial.callout.step2.zoom"), 180.0, 290.0, QSize(280, 92)});
      callouts.push_back({"new_user", tr_key("tutorial.callout.step2.new_user"), 92.0, 290.0, QSize(260, 92)});
      callouts.push_back({"osm_llh", tr_key("tutorial.callout.step2.osm_llh"), 12.0, 290.0, QSize(280, 92)});
      callouts.push_back({"nfz_restricted", tr_key("tutorial.callout.step2.nfz_restricted"), 332.0, 236.0, QSize(248, 86)});
      callouts.push_back({"nfz_warning", tr_key("tutorial.callout.step2.nfz_warning"), 344.0, 274.0, QSize(248, 86)});
      callouts.push_back({"nfz_auth_warn", tr_key("tutorial.callout.step2.nfz_auth_warn"), 344.0, 314.0, QSize(248, 86)});
      callouts.push_back({"nfz_service_white", tr_key("tutorial.callout.step2.nfz_service_white"), 344.0, 352.0, QSize(248, 86)});
      callouts.push_back({"osm_runtime", tr_key("tutorial.callout.step2.osm_runtime"), 18.0, 310.0, QSize(220, 72)});
      callouts.push_back({"osm_stop_btn", tr_key("tutorial.callout.step2.osm_stop_btn"), 340.0, 310.0, QSize(200, 72)});
    } else if (step == 3) {
      callouts.push_back({"bottom_map", tr_key("tutorial.callout.step3.bottom_map"), 178.0, 250.0, QSize(260, 92)});
      ...
      callouts.push_back({"sig_gear", tr_key("tutorial.callout.step5.sig_gear"), 246.0, 260.0, QSize(260, 104)});
      callouts.push_back({"sig_utc", tr_key("tutorial.callout.step5.sig_utc"), 290.0, 248.0, QSize(240, 104)});
      ...
      // Detail Controls (1/2): sats/signal/target/seed + path vmax/acc/prn/maxch
      ...
      callouts.push_back({"seed_slider",  tr_key("tutorial.callout.step7.seed_slider"), 22.0, 300.0, QSize(220, 104)});
  ...
  if ((int)in.osm_status_badge_rects.size() >= 3) {
    zoom_anchor_rect = in.osm_status_badge_rects[0];
    new_user_anchor_rect = in.osm_status_badge_rects[1];
    osm_llh_anchor_rect = in.osm_status_badge_rects[2];
  ...
  // NFZ legend row anchors: use real rects from snapshot if available,
  // else fall back to estimated offsets derived from panel geometry.
  if ((int)in.osm_nfz_legend_row_rects.size() >= 4) {
    nfz_restricted_anchor_rect    = in.osm_nfz_legend_row_rects[0];
    nfz_warning_anchor_rect       = in.osm_nfz_legend_row_rects[1];
    nfz_auth_warn_anchor_rect     = in.osm_nfz_legend_row_rects[2];
    nfz_service_white_anchor_rect = in.osm_nfz_legend_row_rects[3];
  } else if (!in.osm_panel_rect.isEmpty()) {
    const int leg_w = 210;
    const int leg_h = 136;
    ...
    nfz_restricted_anchor_rect    = QRect(leg_x + 10, leg_y + 30, leg_w - 20, 22);
    nfz_warning_anchor_rect       = QRect(leg_x + 10, leg_y + 53, leg_w - 20, 22);
    nfz_auth_warn_anchor_rect     = QRect(leg_x + 10, leg_y + 76, leg_w - 20, 22);
    nfz_service_white_anchor_rect = QRect(leg_x + 10, leg_y + 96, leg_w - 20, 22);
  }
  ...
  if (step == 7 || step == 8) {
    ...
    seed_slider_anchor_rect = detail_text_head_from_row(detail_lo.seed_slider);
    path_v_slider_anchor_rect = rect_from(detail_lo.path_v_slider);
    path_a_slider_anchor_rect = rect_from(detail_lo.path_a_slider);
    prn_slider_anchor_rect = rect_from(detail_lo.prn_slider);
    ch_slider_anchor_rect = rect_from(detail_lo.ch_slider);
  ...
  const auto step1_anchor_from_id = [&](const QString &id) -> QPointF {
    ...
    if (id == "zoom" && !zoom_anchor_rect.isEmpty()) {
      return map_anchor_to_ray_rect(status_text_head(zoom_anchor_rect));
    }
    if (id == "osm_llh" && !osm_llh_anchor_rect.isEmpty()) {
      return map_anchor_to_ray_rect(status_text_head(osm_llh_anchor_rect));
    }
    if (id == "new_user" && !new_user_anchor_rect.isEmpty()) {
      return map_anchor_to_ray_rect(status_text_head(new_user_anchor_rect));
    }
    if (id == "nfz_restricted" && !nfz_restricted_anchor_rect.isEmpty()) {
      return map_anchor_to_ray_rect(nfz_restricted_anchor_rect);
    }
    if (id == "nfz_warning" && !nfz_warning_anchor_rect.isEmpty()) {
      return map_anchor_to_ray_rect(nfz_warning_anchor_rect);
    }
    if (id == "nfz_auth_warn" && !nfz_auth_warn_anchor_rect.isEmpty()) {
      return map_anchor_to_ray_rect(nfz_auth_warn_anchor_rect);
    }
    if (id == "nfz_service_white" && !nfz_service_white_anchor_rect.isEmpty()) {
      return map_anchor_to_ray_rect(nfz_service_white_anchor_rect);
    }
    if (id == "osm_runtime") {
  ...
  const auto step1_target_from_anchor = [&](const QString &id,
    ...
    // NFZ legend: 4 items spread vertically on the right side of the frame
    if (id == "nfz_restricted") {
      // Right side, upper
      const double y = std::max(10.0 + half_h,
                                ray_rect.top() + ray_rect.height() * 0.10);
      return QPointF(right_x, y);
    }
    if (id == "nfz_warning") {
      // Right side, middle
      const double y = std::max(10.0 + half_h,
                                std::min((double)in.win_height - 58.0 - half_h,
                                         ray_rect.top() + ray_rect.height() * 0.38));
      return QPointF(right_x, y);
    }
    if (id == "nfz_auth_warn") {
      // Right side, lower
      const double y = std::max(10.0 + half_h,
                                std::min((double)in.win_height - 58.0 - half_h,
                                         ray_rect.top() + ray_rect.height() * 0.67));
      return QPointF(right_x, y);
    }
    if (id == "nfz_service_white") {
      // Right side, bottom boundary
      const double y = std::max(10.0 + half_h,
                                std::min((double)in.win_height - 58.0 - half_h,
                                         ray_rect.bottom() + 18.0));
      return QPointF(right_x, y);
    }
  ...
  const auto step6_anchor_from_id = [&](const QString &id) -> QPointF {
    ...
    if (id == "seed_slider" && !seed_slider_anchor_rect.isEmpty()) {
      return map_anchor_to_ray_rect(seed_slider_anchor_rect);
    }
  ...
  const auto step6_target_from_anchor = [&](const QString &id,
    ...
    const double lower_y = std::max(10.0 + half_h,
                    std::min((double)in.win_height - 58.0 - half_h,
                         low_y + ((aspect < 1.7) ? -8.0 : 0.0)));

    Q_UNUSED(anchor);

    if (id == "detail_sats") return QPointF(bottom_center_x, top_y);
    if (id == "gain_slider") return QPointF(left_x, upper_y);
    if (id == "cn0_slider") return QPointF(left_x, mid_y);
    if (id == "seed_slider") return QPointF(right_x, upper_y);
    if (id == "path_v_slider") return QPointF(left_x, lower_y);
    if (id == "path_a_slider") return QPointF(right_x, mid_y);
    if (id == "prn_slider") return QPointF(bottom_center_x, bottom_y);
    if (id == "ch_slider") return QPointF(bottom_right_x, bottom_y);
  ...
  for (int i = 0; i < (int)callouts.size() && i < (int)radial.size(); ++i) {
    ...
    if (step == 1) {
      ...
      const double len = std::max(1e-6, std::hypot(v.x(), v.y()));
      const QPointF n(-v.y() / len, v.x() / len);
      const bool compact_line_mode = (step == 5 || step == 6 || step == 7 || step == 8);
      const double bend = compact_line_mode ? std::min(10.0, len * 0.035)
                                            : std::min(24.0, len * 0.08);
      const QPointF ctrl = (anchor + target) * 0.5 + n * bend;
...
TutorialGalaxyGuideStepConfig BuildGuideStepForComponent(
  ...
  static const Mapping kMap[] = {
      ...
      {"sample_rate", "Sample Rate: configure ADC/DSP sampling.", 216.0f, 300.0f},
        ...
        {"seed_input", "Seed Input: set deterministic random seed.", 180.0f, 300.0f},
...
void InjectGuideButtonAnchorsByScene(SunWindowData *central_window,
  ...
  if (scene_id == TutorialImSceneId::Map) {
    ...
    AddFallbackAnchorIfMissing(central_window, "seed_input", 0.28f, 0.74f);
