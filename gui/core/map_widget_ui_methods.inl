...
void MapWidget::clear_hover_help() {
...
int MapWidget::hover_region_for_pos(const QPoint &pos, QString *text,
                                    QRect *anchor) const {
...
void MapWidget::update_hover_help(const QPoint &pos) {
  QString text;
  QRect anchor;
...
void MapWidget::draw_hover_help_overlay(QPainter &p, int win_width,
                                        int win_height) {
  MapHoverHelpInput in;
...
QRect MapWidget::hover_help_invalid_rect(const QRect &anchor) const {
  if (anchor.isEmpty())
    return QRect();
