...
void map_draw_waterfall_panel_expanded(QPainter &p, int win_width, int win_height,
  ...
  draw_panel_title(p, panel_x, panel_y, panel_h, waterfall_title.constData());
  p.setPen(QColor("#c4d2e4"));
  p.drawText(monitor_y_label_rect(draw_rect, draw_rect.top(), 48, 14), Qt::AlignRight | Qt::AlignVCenter, "0dB");
  p.drawText(monitor_y_label_rect(draw_rect, draw_rect.center().y(), 48, 14), Qt::AlignRight | Qt::AlignVCenter, "-15dB");
  p.drawText(monitor_y_label_rect(draw_rect, draw_rect.bottom(), 48, 14), Qt::AlignRight | Qt::AlignVCenter, "-30dB");
...
void map_draw_time_panel_expanded(QPainter &p, int win_width, int win_height,
  ...
  if (in.spec_snap.time_valid && in.spec_snap.time_samples >= 8) {
    QPainterPath path_q, path_i;
    for (int i = 0; i < in.spec_snap.time_samples; ++i) {
      float t = (float)i / (float)(in.spec_snap.time_samples - 1);
      int x = draw_rect.left() + (int)llround(t * (float)(draw_rect.width() - 1));
      float qq = in.spec_snap.time_q[i];
      float ii = in.spec_snap.time_i[i];
      if (qq < -1.0f) qq = -1.0f;
      if (qq > 1.0f) qq = 1.0f;
      if (ii < -1.0f) ii = -1.0f;
      if (ii > 1.0f) ii = 1.0f;
      int yq = draw_rect.center().y() - (int)llround(qq * (float)(draw_rect.height() / 2 - 1));
      int yi = draw_rect.center().y() - (int)llround(ii * (float)(draw_rect.height() / 2 - 1));
      if (i == 0) {
        path_q.moveTo(x, yq);
        path_i.moveTo(x, yi);
      } else {
