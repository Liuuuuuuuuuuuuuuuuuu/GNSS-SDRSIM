...
void control_draw_slider_stacked(QPainter &p,
    ...
    if (show_fs_ticks) {
        ...
        auto draw_tick = [&](double val, const char *lbl) {
            ...
            p.drawLine(tx, ty - track_thick / 2 - 3, tx, ty + track_thick / 2 + 3);
            p.setPen(QColor(139, 195, 255, 255));
            int label_y = std::min(lower.y + lower.h - 2, ty + track_thick / 2 + 13);
            p.drawText(tx - 10, label_y, QString::fromUtf8(lbl));
        };
        p.setRenderHint(QPainter::Antialiasing, false);
        draw_tick(2.6, "2.6");
        draw_tick(5.2, "5.2");
...
void control_draw_three_switch(QPainter &p, const Rect &r, const QColor &border, const QColor &text, const QColor &dim, const QColor &active_fill, const char *caption, const char *a, const char *b, const char *c, int active_idx, bool enabled) {
    ...
    int caption_pt = fit_text_point_size(old_font,
                                         ...
                                         10,
                                         clamp_int(std::max(11, (int)std::round(r.h * 0.52)), 11, 18),
                                         false,
                                         g_caption_text_scale);
    fcap.setPointSize(caption_pt);
    p.setFont(fcap);
    int caption_h = clamp_int(p.fontMetrics().height() + 4, 12, std::max(12, r.h - min_seg_h));
    ...
    p.setPen(t);
    p.drawText(QRect(r.x + 10, base_y + 2, std::max(12, r.w - 20), std::max(10, caption_h - 4)),
               Qt::AlignVCenter | Qt::AlignLeft,
               p.fontMetrics().elidedText(caption_text, Qt::ElideRight, std::max(12, r.w - 12)));
    ...
    for (int i = 0; i < 3; ++i) {
        ...
        int option_pt = fit_text_point_size(old_font,
                                            ...
                                            8,
                                            clamp_int(std::max(9, (int)std::lround((double)seg_h * 0.46)), 9, 18),
                                            i == active_idx,
