...
static inline bool spoof_like_selection(int interference_selection) {
...
static inline void reset_crossbow_gate_state(GuiControlState *ctrl) {
    ...
    ctrl->crossbow_distance_ok = false;
}
...
bool control_logic_handle_click(int x, int y, int win_width, int win_height, GuiControlState *ctrl, std::mutex *ctrl_mtx,
    ...
    compute_control_layout(win_width, win_height, &lo, ctrl->show_detailed_ctrl,
                           ctrl->signal_mode != SIG_MODE_MIXED);
    ...
    if (rect_hit(lo.btn_exit, x, y)) {
        if (ctrl->running_ui) {
            // Runtime EXIT should be immediate: request stop + exit together.
    ...
    if (rect_hit(lo.btn_start, x, y)) {
        if (!ctrl->running_ui && ctrl->interference_selection >= 0) {
            if (spoof_like_selection(ctrl->interference_selection) && !ctrl->spoof_allowed) {
                map_gui_push_alert(2, "__i18n__:alert.spoof_rinex_required");
                return true;
            }
            // Crossbow: allow START with auto-positioned observer, no map point required
            // (removed: if (!ctrl->llh_ready) { map_gui_push_alert(...); return true; })
            ...
            map_gui_reset_monitor_views();  // Reset all four monitor waveforms on START
    ...
    bool spoof_mode = spoof_like_selection(ctrl->interference_selection);
    bool jam_mode = (ctrl->interference_selection == 1);
    bool mode_any = (spoof_mode || jam_mode);
    // Crossbow detail params are restored to behave like spoof mode.
    const bool detail_params_visible = ctrl->show_detailed_ctrl;

    if (ctrl->show_detailed_ctrl && detail_params_visible) {
    ...
    if (ctrl->interference_selection == 1) {
        ...
        if (ctrl->show_detailed_ctrl && detail_params_visible && rect_hit(lo.tg_clk, x, y)) {
