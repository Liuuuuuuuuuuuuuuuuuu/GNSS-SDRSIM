...
bool control_logic_handle_click(int x, int y, int win_width, int win_height, GuiControlState *ctrl, std::mutex *ctrl_mtx,
    ...
    if (rect_hit(lo.btn_start, x, y)) {
        if (!ctrl->running_ui && (ctrl->llh_ready || ctrl->interference_mode)) {
            ctrl->running_ui = true; start_req->fetch_add(1); return true;
        }
    }
    ...
    if (ctrl->show_detailed_ctrl) {
        ...
        if (fmt_idx >= 0) { ctrl->byte_output = (fmt_idx == 1); return true; }
    }

    int jam_idx = segmented_index_hit(lo.sw_jam, x, y, 2);
    if (jam_idx >= 0) {
        ctrl->interference_mode = (jam_idx == 1);
        if (ctrl->interference_mode && ctrl->signal_mode == SIG_MODE_GPS && ctrl->fs_mhz < 2.6) {
            ctrl->fs_mhz = 2.6;
        }
        return true;
    }
