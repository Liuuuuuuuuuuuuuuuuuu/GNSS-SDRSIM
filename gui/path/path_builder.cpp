...
static bool fetch_osrm_route_polyline_on_current_thread(double lat0_deg, double lon0_deg,
                                      ...
                                      std::vector<LonLat> *out_polyline)
{
    ...
    if (app && QThread::currentThread() != app->thread()) {
        ...
        QMetaObject::invokeMethod(app,
                                  [&]() {
                                      ok = fetch_osrm_route_polyline_on_current_thread(
                                          lat0_deg, lon0_deg, lat1_deg, lon1_deg, out_polyline);
                                  },
                                  Qt::BlockingQueuedConnection);
        return ok;
    }

    return fetch_osrm_route_polyline_on_current_thread(lat0_deg, lon0_deg, lat1_deg,
                                                       lon1_deg, out_polyline);
}
