...
/* Declared in globals.c; setting this causes generate_signal() to exit promptly. */
extern volatile int g_runtime_abort;

static uhd::usrp::multi_usrp::sptr usrp_dev;

/* rid_rx.cpp 使用此函式存取共享裝置將柄 */
uhd::usrp::multi_usrp::sptr usrp_get_dev(void) { return usrp_dev; }
...
static std::thread tx_async_worker_thread;
static std::atomic<bool> tx_async_running(false);

/* Abort flag set by our SIGINT/SIGTERM handler.  tx_worker checks this
 * every 20 ms so it can exit quickly and send a clean EOB even on Ctrl+C. */
static std::atomic<bool> g_abort_fast{false};
static struct sigaction g_prev_sigint;
static struct sigaction g_prev_sigterm;
static bool g_signal_handlers_installed = false;

static void usrp_signal_handler(int sig)
{
    /* Signal-safe: only write atomics */
    g_abort_fast.store(true, std::memory_order_release);
    g_runtime_abort = 1;   /* make generate_signal() stop within one ms */
    (void)sig;
}
...
static void emit_async_status_char(const uhd::async_metadata_t &md_async) {
    char c = '\0';
    switch (md_async.event_code) {
...
static void tx_async_worker() {
    while (tx_async_running.load(std::memory_order_relaxed)) {
        if (!tx_stream) {
            std::this_thread::sleep_for(std::chrono::milliseconds(20));
            continue;
        }
        uhd::async_metadata_t async_md;
        if (tx_stream->recv_async_msg(async_md, 0.05)) {
            emit_async_status_char(async_md);
        }
    }
}
...
void tx_worker() {
    ...
    while (is_running || !tx_queue.empty()) {
        /* --- Abort path: Ctrl+C or forced stop -------------------------------- */
        if (g_abort_fast.load(std::memory_order_acquire)) {
            {
                std::lock_guard<std::mutex> lock(mtx);
                clear_tx_queue_locked();
                is_running = false;
            }
            cv_producer.notify_all(); /* wake usrp_send so it returns 0 */
            /* Send immediate 0-sample EOB so hardware stops cleanly */
            if (tx_stream) {
                uhd::tx_metadata_t eob;
                eob.start_of_burst = false;
                eob.end_of_burst   = true;
                eob.has_time_spec  = false;
                int16_t dummy = 0;
                try { tx_stream->send(&dummy, 0, eob); } catch (...) {}
            }
            return; /* exit tx_worker */
        }
...

        {
            ...
            /* Use wait_for so we re-check the abort flag every 20 ms */
            cv_consumer.wait_for(lock, std::chrono::milliseconds(20),
                [] { return !tx_queue.empty() || !is_running; });
        ...
        if (tx_stream) {
            ...
            // Check if this is the last buffer in the queue and running has stopped
            bool is_last_buffer = false;
            {
                std::lock_guard<std::mutex> lock(mtx);
                is_last_buffer = (!is_running && tx_queue.empty());
            }
            ...
            while (num_tx_samps < total_samps) {
                ...
                // Set EOB on the very last sample block so USRP gets a clean burst end.
                bool this_is_last = is_last_buffer && ((num_tx_samps + tx_stream->get_max_num_samps()) >= total_samps);
                md.end_of_burst = this_is_last;
                ...
                md.end_of_burst = false; // reset after send
                if (sent == 0) {
                    break;
                }
                num_tx_samps += sent;
...
int usrp_open_dev_only(void) {
    if (usrp_dev) return 0;  /* 裝置已開啟，直接回傳 */
    try {
        usrp_dev = uhd::usrp::multi_usrp::make(uhd::device_addr_t(""));
        usrp_dev->set_clock_source("internal");
        fprintf(stderr, "[usrp] 裝置已開啟 (RX-only 模式)\n");
        return 0;
    } catch (const std::exception &e) {
        fprintf(stderr, "[usrp] RX-only 裝置開啟失敗: %s\n", e.what());
        return -1;
    }
}
    ...
    try {
        ...
        md.has_time_spec  = false; 
        {
            ...
            is_running = true;
                ...
                /* --- Install signal handlers AFTER device is ready ------------------- */
        {
            struct sigaction sa;
            std::memset(&sa, 0, sizeof(sa));
            sa.sa_handler = usrp_signal_handler;
        ...
        tx_async_running.store(true, std::memory_order_relaxed);
        tx_async_worker_thread = std::thread(tx_async_worker);
...
void usrp_close(void) {
    ...
    // ── 步驟 1：通知 tx_worker 停止接受新資料，等它把 queue 送完（含帶 EOB 的最後一包）
    {
        std::lock_guard<std::mutex> lock(mtx);
        is_running = false;
    }
    ...
    if (tx_worker_thread.joinable()) {
        tx_worker_thread.join(); // 等到硬體實際發完最後一包（帶 EOB）才繼續
    }

    // ── 步驟 2：硬體已停止發射。停止 async 監聽執行緒（不再印任何字元）
    tx_async_running.store(false, std::memory_order_relaxed);
    if (tx_async_worker_thread.joinable()) {
        tx_async_worker_thread.join();
    }

    /* Restore original SIGINT/SIGTERM handlers so the caller can handle
     * further signals normally (e.g. a second Ctrl+C terminates the process). */
    ...
    g_abort_fast.store(false, std::memory_order_relaxed);

    // ── 步驟 3：靜默 drain 剩餘 async 事件（ACK / 殘留 SEQ_ERROR），避免 UHD teardown 自行印出
    if (tx_stream) {
        uhd::async_metadata_t drain_md;
        auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(200);
        while (std::chrono::steady_clock::now() < deadline) {
            if (!tx_stream->recv_async_msg(drain_md, 0.02)) break;
        }
        tx_stream.reset();
    }
    ...
    std::cout << "[USRP] 硬體安全關閉完成。" << std::endl;
}
