...
/* Keep shared accessor for rid_rx.cpp in current tree. */
...
static void drain_tx_async_reports_ms(int max_wait_ms) {
    if (!tx_stream || max_wait_ms <= 0) return;

    const auto deadline = std::chrono::steady_clock::now() + std::chrono::milliseconds(max_wait_ms);
    uhd::async_metadata_t async_md;
    while (std::chrono::steady_clock::now() < deadline) {
        // Short poll loop: drain ACK/underflow events so control path can settle
        // before streamer/device teardown.
        if (!tx_stream->recv_async_msg(async_md, 0.02)) {
            break;
        }
    }
}
...
int usrp_init(double freq, double rate, double gain, int use_external_clk, int sample_format) {
    ...
    for (int attempt = 1; attempt <= 2; ++attempt) {
        try {
            ...
            if (use_external_clk) {
                ...
                if (!locked) {
                    std::cerr << "[USRP] 警告: external ref lock 未確認，仍嘗試持續 發射" << std::endl;
                }
            }
            ...
            if (attempt == 1 && last_err.find("accum_timeout < _timeout") != std::string::npos) {
                std::cerr << "[USRP][warn] 初始化遇到暫時性 ACK timeout，0.8 秒後重試一次..." << std::endl;
    ...
    std::cerr << "❌ [USRP] 初始化錯誤: " << last_err << std::endl;
    return -1;
}
...
void usrp_close_fast(void) {
    ...
    std::cout << "[USRP] 快速退出關閉完成。" << std::endl;
}
...
void usrp_close(void) {
    ...
    if (tx_stream) {
        try {
            ...
            md.has_time_spec = false;
            int16_t dummy = 0;
            // Tell FPGA TX path to terminate burst explicitly.
            tx_stream->send(&dummy, 0, md);
            // Drain async control reports in two stages so late ACK/SEQ packets
            // are consumed before streamer/device teardown.
            drain_tx_async_reports_ms(320);
            std::this_thread::sleep_for(std::chrono::milliseconds(80));
            drain_tx_async_reports_ms(120);
        } catch (const std::exception &e) {
            std::cerr << "[USRP] 關閉串流時收到例外: " << e.what() << std::endl;
        } catch (...) {
            std::cerr << "[USRP] 關閉串流時收到未知例外" << std::endl;
        }
        tx_stream.reset();
    }
    // Give UHD control path a short settle window before releasing device.
