...
static std::atomic<int> g_cuda_alloc_fault_latched{0};

static bool should_log_cuda_alloc_failure_once(void)
{
    return g_cuda_alloc_fault_latched.exchange(1) == 0;
}
...
void reset_signal_engine_state(void)
{
    ...
    g_cuda_alloc_fault_latched.store(0);
}
                                         ...
                                         double start_bdt)
{
    ...
    {
        cudaError_t e1 = cudaMalloc((void **)&d_iq_out, iq_bytes);
        cudaError_t e2 = cudaMalloc((void **)&d_jam_ch, (size_t)n_jam_ch * sizeof(jam_prn_state_t));
        cudaError_t e3 = cudaMalloc((void **)&d_spec_db, (size_t)GUI_SPECTRUM_BINS * sizeof(float));
        cudaError_t efirst = (e1 != cudaSuccess) ? e1 : (e2 != cudaSuccess) ? e2 : e3;
        if (efirst != cudaSuccess) {
            if (should_log_cuda_alloc_failure_once()) {
                fprintf(stderr,
                        "[cuda][error] JAM GPU 記憶體分配失敗: %s\n"
                        "[cuda][hint] 已停用後續 GPU 分配嘗試以避免重複刷屏；請先修復 CUDA runtime/driver 後再重新啟動。\n",
                        cudaGetErrorString(efirst));
            }
...
static void check_ephemeris_age(int week,double sow)
{
    ...
    for(int prn=1; prn<=prn_max; ++prn){
        ...
        if(is_geo_prn(prn)) continue;   /* GEO 不參與模擬，略過 toe 檢查 */
        double toe = eph[prn].week*604800.0 + eph[prn].toe;
        double diff = fabs(t - toe);
        if(diff > 2*86400.0){
            fprintf(stderr,
                    "[warn] PRN%02d toe %.1f days — 星曆過舊，已從本次模擬排除\n",
                    prn, diff/86400.0);
            eph[prn].prn = 0;   /* 排除此衛星，避免 tk 誤差過大 */
            excluded = 1;
        }
    }
    if(excluded)
        fputs("[warn] 部分衛星因星曆過舊已排除，建議更新 BRDM 檔案\n", stderr);
}
...
void generate_signal(const sim_config_t *cfg)
{
    ...
    if (utc_to_bdt(cfg->time_start, &usr.week, &usr.sow) != 0) {
        /* Fallback to host UTC to avoid repeated hard-stop loops on bad input. */
        ...
        if (utc_to_bdt(now_utc, &usr.week, &usr.sow) != 0) {
            fputs("UTC format err (fallback failed)\n", stderr);
            free_path(&path);
            return;
        }

        static int s_utc_warn_once = 0;
        if (!s_utc_warn_once) {
            fprintf(stderr,
                    "UTC format err: '%s' (fallback to host UTC %s)\n",
                    cfg->time_start, now_utc);
            s_utc_warn_once = 1;
        }
    }
    ...
    // =========================================================
    channel_t* d_channels = NULL;
    int16_t *d_iq_out = NULL;
    int16_t *host_iq = NULL;
    float *d_spec_db = NULL;
    size_t iq_bytes = (size_t)(2 * samp_per_ms) * sizeof(int16_t);
    {
        cudaError_t e1 = cudaMalloc((void**)&d_channels, MAX_CH * sizeof(channel_t));
        cudaError_t e2 = cudaMalloc((void**)&d_iq_out, iq_bytes);
        cudaError_t e3 = cudaMallocHost((void**)&host_iq, iq_bytes);
        cudaError_t e4 = cudaMalloc((void**)&d_spec_db, GUI_SPECTRUM_BINS * sizeof(float));
        cudaError_t efirst = (e1 != cudaSuccess) ? e1 :
                             (e2 != cudaSuccess) ? e2 :
                             (e3 != cudaSuccess) ? e3 : e4;
        if (efirst != cudaSuccess) {
            if (should_log_cuda_alloc_failure_once()) {
                fprintf(stderr, "[cuda][error] GPU 記憶體分配失敗: %s\n"
                                "[cuda][hint] 請確認 CUDA driver 正常 (嘗試: sudo nvidia-modprobe && sudo reboot)\n"
                                "[cuda][hint] 已停用後續 GPU 分配錯誤重複輸出，避免 console 被刷屏。\n",
                        cudaGetErrorString(efirst));
            }
            if (d_spec_db)  cudaFree(d_spec_db);
            if (host_iq)    cudaFreeHost(host_iq);
            if (d_iq_out)   cudaFree(d_iq_out);
            if (d_channels) cudaFree(d_channels);
            free_path(&path);
            return;
        }
    }
