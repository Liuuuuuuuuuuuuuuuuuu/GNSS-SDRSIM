...
int read_rinex_nav(const char *fname_bds, const char *fname_gps, double start_bdt)
{
    double start_gpst = bdt_seconds_to_gpst_seconds(start_bdt);
    int ret_bds = (fname_bds && fname_bds[0]) ? read_rinex_nav_bds(fname_bds, start_bdt) : -1;
    int ret_gps = (fname_gps && fname_gps[0]) ? read_rinex_nav_gps(fname_gps, start_gpst) : -1;

    if (ret_bds != 0)
        fprintf(stderr, "[rinex] 新星曆不含 BDS (C) 記錄，沿用上一版 BDS 星曆\n");
    if (ret_gps != 0)
        fprintf(stderr, "[rinex] 新星曆不含 GPS (G) 記錄，沿用上一版 GPS 星曆\n");
